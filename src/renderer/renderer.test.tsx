...
describe('JSX', () => {
  ...
  it('should catch error with number object', () => {
    const tree = renderer.create(<JSXRenderer disableKeyGeneration code={`<p>{raise()}</p><p />`} binding={{ raise: () => { throw 1; }}} />).toJSON();
    expect(tree).toMatchSnapshot();
  })

  it('should catch error with object', () => {
    const tree = renderer.create(<JSXRenderer disableKeyGeneration code={`<p>{raise()}</p>`} binding={{ raise: () => { throw { foo: 1 }; }}} />).toJSON();
    expect(tree).toMatchSnapshot();
  })

  const elementFilters: JSXElementFilter[] = [
    (e) => e.component === 'link' ? undefined : e,
    (e) => e.component === 'script' ? undefined : e,
  ];
  ...
  const textFilters: JSXTextFilter[] = [
    (text) => text === 'ban' ? undefined : text,
  ];
    ...
    it(`should render ${code}`, () => {
      const actual = renderer.create(
        <JSXRenderer
          code={code}
          binding={{
            exception() { throw new Error('Hello') }
          }}
