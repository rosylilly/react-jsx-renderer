...
export const evalExpression = (exp: ESTree.Expression, context: EvaluateContext): any => {
  switch (exp.type) {
    case 'ArrayExpression': return evalArrayExpression(exp, context);
    case 'ArrayPattern': return evalArrayPattern(exp, context);
    case 'ArrowFunctionExpression': return evalArrowFunctionExpression(exp, context);
    case 'AssignmentExpression': return evalAssignmentExpression(exp, context);
    case 'AwaitExpression': return evalAwaitExpression(exp, context);
    case 'BinaryExpression': return evalBinaryExpression(exp, context);
    case 'CallExpression': return evalCallExpression(exp, context);
    case 'ChainExpression': return evalChainExpression(exp, context);
    case 'ClassDeclaration': return evalClassDeclaration(exp, context);
    case 'ClassExpression': return evalClassExpression(exp, context);
    case 'ConditionalExpression': return evalConditionalExpression(exp, context);
    case 'FunctionExpression': return evalFunctionExpression(exp, context);
    case 'Identifier': return evalIdentifier(exp, context);
    case 'Import': return evalImport(exp, context);
    case 'ImportExpression': return evalImportExpression(exp, context);
    case 'JSXClosingElement': return evalJSXClosingElement(exp, context);
    case 'JSXClosingFragment': return evalJSXClosingFragment(exp, context);
    case 'JSXElement': return evalJSXElement(exp, context);
    case 'JSXExpressionContainer': return evalJSXExpressionContainer(exp, context);
    case 'JSXFragment': return evalJSXFragment(exp, context);
    case 'JSXOpeningElement': return evalJSXOpeningElement(exp, context);
    case 'JSXOpeningFragment': return evalJSXOpeningFragment(exp, context);
    case 'JSXSpreadChild': return evalJSXSpreadChild(exp, context);
    case 'Literal': return evalLiteral(exp, context);
    case 'LogicalExpression': return evalLogicalExpression(exp, context);
    case 'MemberExpression': return evalMemberExpression(exp, context);
    case 'MetaProperty': return evalMetaProperty(exp, context);
    case 'NewExpression': return evalNewExpression(exp, context);
    case 'ObjectExpression': return evalObjectExpression(exp, context);
    case 'ObjectPattern': return evalObjectPattern(exp, context);
    case 'RestElement': return evalRestElement(exp, context);
    case 'SequenceExpression': return evalSequenceExpression(exp, context);
    case 'SpreadElement': return evalSpreadElement(exp, context);
    case 'Super': return evalSuper(exp, context);
    case 'TaggedTemplateExpression': return evalTaggedTemplateExpression(exp, context);
    case 'TemplateLiteral': return evalTemplateLiteral(exp, context);
    case 'ThisExpression': return evalThisExpression(exp, context);
    case 'UnaryExpression': return evalUnaryExpression(exp, context);
    case 'UpdateExpression': return evalUpdateExpression(exp, context);
    case 'YieldExpression': return evalYieldExpression(exp, context);
    default:
...
export const evalBinaryExpression = (exp: ESTree.BinaryExpression, context: EvaluateContext) => {
  ...
  switch (exp.operator) {
    ...
    case '|': return left() | right();
    case '^': return left() ^ right();
    default: throw new EvaluateError(`Unknown binary operator: ${exp.operator}`, exp, context);
  }
}
...
export const evalCallExpression = (exp: ESTree.CallExpression, context: EvaluateContext) => {
  ...
  const method = evalExpression(callee, context) as Function;
...
export const evalLogicalExpression = (exp: ESTree.LogicalExpression, context: EvaluateContext) => {
  ...
  switch (exp.operator) {
    ...
    case '||': return left() || right();
    case '??': return left() ?? right();
    default: throw new EvaluateError(`Unknown logical operator: ${exp.operator}`, exp, context);
  }
}
...
export const evalMemberExpression = (exp: ESTree.MemberExpression, context: EvaluateContext) => {
  ...
  const key = (
    ...
    property.type === 'PrivateIdentifier' ? property.name :
    evalExpression(property, context)
  );
...
export const evalTemplateLiteral = (exp: ESTree.TemplateLiteral, context: EvaluateContext) => {
  return [...exp.expressions, ...exp.quasis].sort((a, b) => (a.start || 0) - (b.start || 0)).map((e) => {
    switch (e.type) {
      case 'TemplateElement': return e.value.cooked
      default: return evalExpression(e, context);
    }
  }).join('');
}
...
export const evalUnaryExpression = (exp: ESTree.UnaryExpression, context: EvaluateContext) => {
  switch(exp.operator) {
    case '+': return +(evalExpression(exp.argument, context));
    case '-': return -(evalExpression(exp.argument, context));
    case '~': return ~(evalExpression(exp.argument, context));
    case '!': return !(evalExpression(exp.argument, context));
    case 'void': return void(evalExpression(exp.argument, context));
    // case 'delete': return delete this.evalExpression(expression.argument);
    case 'typeof': return typeof evalExpression(exp.argument, context);
    default: throw new EvaluateError(`Unknown unary operator: ${exp.operator}`, exp, context);
  }
}
...
export const evalUpdateExpression = (exp: ESTree.UpdateExpression, context: EvaluateContext) => {
  switch (exp.operator) {
    case '++': // TODO: Implement
    case '--': // TODO: Implement
    default: throw new EvaluateError(`Unknown update operator: ${exp.operator}`, exp, context);
  }
}
...
export const evalJSXChild = (jsx: ESTree.JSXChild, context: EvaluateContext): JSXNode => {
  switch (jsx.type) {
    case 'JSXEmptyExpression': return evalJSXEmptyExpression(jsx, context);
    case 'JSXText': return evalJSXText(jsx, context);
    ...
    default: return evalExpression(jsx, context);
  }
}
...
export const evalJSXExpressionContainer = (jsx: ESTree.JSXExpressionContainer, context: EvaluateContext): any => {
  ...
  switch (expression.type) {
    case 'JSXEmptyExpression': return evalJSXEmptyExpression(expression, context);
    default: return evalExpression(expression, context);
  }
}
...
export const evalJSXTagNameExpression = (jsx: ESTree.JSXTagNameExpression, context: EvaluateContext): string => {
  switch (jsx.type) {
    case 'JSXIdentifier': return evalJSXIdentifier(jsx, context);
    case 'JSXMemberExpression': return evalJSXMemberExpression(jsx, context);
    case 'JSXNamespacedName': return evalJSXNamespacedName(jsx, context);
  }
}
...
export const evalJSXAttributeValue = (jsx: ESTree.JSXAttributeValue, context: EvaluateContext) => {
  ...
  switch (jsx.type) {
    case 'JSXIdentifier': return evalJSXIdentifier(jsx, context);
    case 'Literal': return evalLiteral(jsx, context);
    case 'JSXElement': return evalJSXElement(jsx, context);
    case 'JSXFragment': return evalJSXFragment(jsx, context);
    case 'JSXExpressionContainer': return evalJSXExpressionContainer(jsx, context);
    case 'JSXSpreadChild': return evalJSXSpreadChild(jsx, context);
